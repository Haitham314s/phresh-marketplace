...
from tests.shared.offers import new_accepted_cleaning_offer


offer_in = {"status": OfferStatus.cancelled}
...
async def test_cancel_accepted_offer(create_authorized_client: Callable, test_users: list[User]):
    ...
    cleaning = await new_accepted_cleaning_offer(test_users[0], test_users[1:])
    offer = [
...
async def test_other_offers_when_cancelled(create_authorized_client: Callable, test_users: list[User]):
    ...
    res = await authorized_client.put(f"/cleaning/{cleaning.id}/offer/{accepted_offer.id}", json=offer_in)
    assert res.status_code == status.HTTP_200_OK
